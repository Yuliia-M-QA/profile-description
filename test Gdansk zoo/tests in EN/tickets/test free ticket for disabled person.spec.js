...
test('test', async ({ page }) => {
  ...
  await page1.getByRole('row', { name: 'Free ticket for a disabled' }).getByRole('button').nth(1).click();
...
//   await page1.getByRole('checkbox', { name: 'I declare that I have read' }).check();
  await page1.locator('#koszyk_wartosc').click();
  await page1.getByText('Tickets in basketZOO visit -').click();
  await page1.getByRole('button', { name: 'en:ODBIERAM' }).click();
});